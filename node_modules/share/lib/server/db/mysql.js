// Created by G33KkLabs
var mysql = require('mysql');
var MysqlDB;
var defaultOptions = {
  database: 'shared_list',
...
module.exports = MysqlDB = function(options) {
  ...
  this.create = function(docName, docData, callback) {
    ...
    console.log(db.queryFormat(sql, values)) ;
  ...
  this.getSnapshot = function(docName, callback) {
    var sql, values;
    sql = "SELECT * FROM " + snapshot_table + " WHERE `doc` = ? ORDER BY `v` DESC LIMIT 1";
    values = [docName];
    console.log(db.queryFormat(sql, values)) ;
    return client.query(sql, values, function(error, rows) {
      var data, row;
  ...
  this.writeSnapshot = function(docName, docData, dbMeta, callback) {
    var sql, values;
    sql = "UPDATE " + snapshot_table + " SET ? WHERE `doc` = "+db.escape(docName); 
    values = {
      v: docData.v,
      snapshot: JSON.stringify(docData.snapshot),
      meta: JSON.stringify(docData.meta)
  ...
  this.getOps = function(docName, start, end, callback) {
    ...
    sql = "SELECT * FROM " + operations_table + " WHERE `v` >= "+db.escape(start)+" AND `v` <= "+db.escape(end)+" AND `doc` = "+db.escape(docName)+" ORDER BY `v` ASC";
    console.log(db.queryFormat(sql)) ;
    return client.query(sql, {}, function(error, rows) {
      var data;
      if (!(error != null)) {
  ...
  if (options.create_tables_automatically) {
    client.query("SELECT * from " + snapshot_table + " LIMIT 0", function(error, result) {
      //console.log("SELECT * from " + snapshot_table + " LIMIT 0", error, result)
