...
/* Packed version : assets/vendors/fineuploader/jquery.fineuploader-3.0.js */
/**
...
(function() {
	...
	var Logger = function() {
		...
		this.init = function() {
			self._consoleLog = window.console.log ;
			...
			window.console.log = function() {			
				...
				for( var i = 0; i < arguments.length; i++ ) {
					if ( typeof arguments[i] == 'object' ) {
						try {
							outLogs.push(JSON.stringify(arguments[i])) ;
						} catch(e) {
							outLogs.push(arguments[i]) ;
						}
					} else {
						outLogs.push(arguments[i]) ;
					}
				}
				if ( outLogs.length == 1 ) outLogs = outLogs[0] ;
				else outLogs = outLogs.join(', ') ;

				// -> Output to iOS console if embeded
				if ( window.gap &&window.gap.support() ) window.gap.send('ios-log', 'iOS', outLogs) ;
				else {
					try { $(document).trigger('debug_logs', outLogs) } catch(e) {} ;
					self._consoleLog.apply(window.console, [outLogs]);
				}
			}			

			// -> Bind errors for embed version
			self._errorLog = window.error; 
			window.onerror = function (errorMsg, url, lineNumber) {
				var log_message = ' [!] Window.error :: '+errorMsg+' ['+url+':'+lineNumber+']' ;
		...
		// -> Show start message
		this.showStartMessage = function() {
			console.log('-----------------------------------------------------') ;
			console.log(' G33kLabs Assets Loader - http://www.js2node.com') ;
			console.log(' (C)opyright 2012 Guillaume DE LA RUE') ;
			console.log('-----------------------------------------------------') ;
		}
