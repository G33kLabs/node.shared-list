(function () {
    ...
    $(document).ready(function () {
        ...
        var connection = null; 
        var activeDoc = null;
        app.openShareList = function(title) { 
        ...
        async.series([
        	function(callback) {
        		require(['http://ajaxorg.github.com/ace/build/src/ace.js'], function() {
        			callback(null);
        		})
        	},
        	function(callback) {
	...
	$(window).on('hashchange', function () { //detect hash change
	    ...
	    if ( hash && hash != 'home' ) {
	    	if ( hash == 'new' ) {
	    		hash = app.generateId(24) ;
	    		$('#editor h4').html('Create a new list')
	    		window.location.hash = hash;
	    		return;
	    	}
	    	app.openShareList(hash) ;	
